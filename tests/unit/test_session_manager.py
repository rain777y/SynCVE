...
class TestLogData:
    ...
    def test_log_data_persists_frame_ledger(self, mock_supabase, sample_analysis_result, sample_session_id):
        session_manager._session_frame_indices.pop(sample_session_id, None)

        result = session_manager.log_data(
            sample_session_id,
            sample_analysis_result,
            metadata={
                "client_capture_ts": "2026-05-10T01:00:00.000Z",
                "client_frame_id": 7,
                "server_received_ts": "2026-05-10T01:00:00.100Z",
        ...
        ledger = result["frame_ledger"]
        assert ledger["frame_idx"] == 0
        assert ledger["client_frame_id"] == 7
        assert ledger["timing"]["inference_latency_ms"] == 123
        assert ledger["raw"]["confidence_norm"] == pytest.approx(0.85, abs=0.001)
        assert ledger["quality_flags"]["low_confidence"] is False

        inserted = mock_supabase.table.return_value.insert.call_args.args[0]
        assert inserted["metadata"]["frame_ledger"]["frame_idx"] == 0
        assert inserted["raw_payload"]["frame_ledger"]["raw"]["dominant_emotion"] == "happy"
...
class TestAggregateEmotionMetrics:
    ...
    def test_deepface_percentage_scores_normalized(self):
        """0-100 frames should be normalised to probabilities."""
        data = [{"emotions": {"happy": 90, "sad": 5, "angry": 5}}]
        metrics = session_manager.aggregate_emotion_metrics(data)
        # 90/100 = 0.9
        assert metrics["averages"]["happy"] == pytest.approx(0.9, abs=0.01)

    def test_deepface_small_percentage_scores_normalized_per_frame(self):
        """Small percentages like 1.2 must not become 120% in reports."""
        data = [{"emotions": {"happy": 98.2, "sad": 1.2, "angry": 0.6}}]
        metrics = session_manager.aggregate_emotion_metrics(data)
        assert metrics["averages"]["happy"] == pytest.approx(0.982, abs=0.001)
        assert metrics["averages"]["sad"] == pytest.approx(0.012, abs=0.001)
        assert metrics["peak_score"] <= 1.0

    def test_scores_already_normalized(self):
        """Already-normalised probability frames should be kept as-is."""
...
class TestGetSessionDetails:
    def test_returns_session(self, mock_supabase):
        result = session_manager.get_session_details("11111111-1111-1111-1111-111111111111")
    ...
    def test_session_not_found(self, mock_supabase):
        ...
        result = session_manager.get_session_details("22222222-2222-2222-2222-222222222222")
        assert "error" in result

    def test_no_supabase_returns_error(self, monkeypatch):
        monkeypatch.setattr("src.backend.storage._supabase_client", None)
        monkeypatch.setattr("src.backend.storage._supabase_initialized", True)
        result = session_manager.get_session_details("33333333-3333-3333-3333-333333333333")
        assert "error" in result
