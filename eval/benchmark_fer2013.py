"""
eval/benchmark_fer2013.py — FER2013 Baseline Evaluation Runner

Evaluates DeepFace emotion recognition on the FER2013 test split.
Produces classification report, confusion matrix, ROC/AUC, latency stats,
and publication-quality plots.

Usage
-----
    python -m eval.benchmark_fer2013 --limit 100          # quick test run
    python -m eval.benchmark_fer2013                      # full evaluation
    python -m eval.benchmark_fer2013 --detector opencv    # alternate detector
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Benchmark DeepFace on FER2013 test set",
    )
    parser.add_argument(
        "--dataset",
        type=str,
...
def run_benchmark(args: argparse.Namespace) -> None:
    ...
    if args.limit > 0:
        # Stratified sampling: maintain class proportions when limiting
    ...
    y_true: list[str] = []
    y_pred: list[str] = []
    y_scores: list[list[float]] = []   # normalised probabilities per sample
    latencies: list[float] = []
    failures: list[dict] = []

    print(f"\nRunning DeepFace.analyze  |  detector={args.detector}")
    for img_path, true_label in tqdm(samples, desc="FER2013 eval", unit="img"):
        ...
        try:
            ...
            # DeepFace returns a list of face dicts
            face = result[0] if isinstance(result, list) else result
            emotion_dist = face["emotion"]  # {"angry": 5.2, "happy": 80.1, ...}
            pred_label = face["dominant_emotion"]

            # Normalise scores to [0, 1] (DeepFace returns 0-100)
    ...
    # Compact AUC dict for JSON (just the AUC numbers)
    ...
    # ------------------------------------------------------------------
    print("\n" + "=" * 60)
    print("FER2013 BASELINE EVALUATION SUMMARY")
    print("=" * 60)
