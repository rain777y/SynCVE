...
from src.backend.uncertainty_fusion import (
    aggregate_emotions as fusion_aggregate_emotions,
)
...
def _aggregate_emotions(
    ...
    """
    Combine emotion probabilities from multiple detector runs.

    Delegates to ``uncertainty_fusion.aggregate_emotions`` which implements
    Axis 1C (entropy-weighted softmax fusion). The legacy fixed-weight
    behaviour is preserved by setting ``settings.fusion.method = "fixed"``.
    The supplied ``weights`` list (computed by ``_normalize_weights`` from
    ``DETECTOR_WEIGHTS``) is forwarded as the ``fixed_weights`` prior, so
    cohort-tuned weighting can be blended with the entropy-derived weights.
    """
    fcfg = get_config().fusion
    return fusion_aggregate_emotions(
        backend_results,
        detectors,
        method=fcfg.method,
        temperature=fcfg.temperature,
        fixed_weights=weights,
        entropy_floor=fcfg.entropy_floor,
        blend_with_fixed=fcfg.blend_with_fixed,
...
def analyze(
    ...
    try:
        ...
        if ensemble_enabled and len(detectors) > 1:
            ...
            successful_detectors = [detector_backend]
        ...
        result["analysis_meta"] = {
            ...
            "detector_backend": detector_backend,
            "ensemble_enabled": bool(ensemble_enabled and len(detectors) > 1),
            "align": align_flag,
            "anti_spoofing": anti_spoofing_flag,
