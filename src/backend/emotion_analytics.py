...
def aggregate_emotion_metrics(
    ...
    for entry in raw_vision_data:
        ...
        for emotion_label, raw_score in scores.items():
            try:
                frame_scores[emotion_label] = float(raw_score)
            except (TypeError, ValueError):
                continue
        if not frame_scores:
            continue

        # DeepFace emits a full 0-100 percentage vector whose entries sum to
        # roughly 100. Deciding scale per value is wrong because small
        # percentages such as 0.8 or 1.2 look like normalised probabilities.
        # Decide once per frame, then clamp into probability space.
        total_score = sum(max(score, 0.0) for score in frame_scores.values())
        is_percentage_frame = (
            total_score > 1.5 or any(score > 1.0 for score in frame_scores.values())
        )

        samples += 1
        for emotion_label, score in frame_scores.items():
            if is_percentage_frame:
                score = score / 100.0
