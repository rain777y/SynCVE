...
Transforms per-frame emotion snapshots into temporal intelligence:
  ...
  - **Event-level change-point detection** (Axis 1A) — multi-method
    consensus over sliding-window z-score, CUSUM, and PELT delegated
    to ``event_detector.EventDetector``.
...
class TemporalAnalyzer:
    ...
    def __init__(
        ...
        # Axis 1A — multi-method change-point event detector. Optional so
        # legacy callers that only want EMA + transitions can still
        # construct the analyzer without paying the import cost (None
        # falls back gracefully in add_frame / get_session_summary).
        self.event_detector: Optional[EventDetector] = event_detector
        # When provided externally, surface the per-frame ensemble payload
        # so clinical_metrics can compute per-detector reliability.
    ...
    def add_frame(
        ...
        # Axis 1A — feed the smoothed vector into the event detector.
        if self.event_detector is not None:
            try:
                self.event_detector.detect_streaming(
                    smoothed_snapshot,
                    frame_idx=self._frame_count,
                    timestamp=timestamp,
                )
            except Exception:
                # Detector failures must never break the analyse path.
                pass

        if ensemble_meta is not None:
            self._ensemble_history.append({"ensemble": ensemble_meta})
    ...
    def get_events(self) -> List[Dict]:
        """
        Return the consensus events confirmed so far. If no detector is
        attached, returns an empty list. The detector is run against the
        full smoothed history so streaming and post-hoc results agree.
        """
        if self.event_detector is None:
            return []
        evs: List[DetectedEvent] = self.event_detector.detect_batch(
            list(self._smoothed_history),
            timestamps=list(self._timestamps),
        )
        return [event_to_dict(e) for e in evs]

    def get_smoothed_history_dicts(self) -> List[Dict[str, float]]:
        """Public accessor used by clinical_metrics post-hoc."""
        return [dict(h) for h in self._smoothed_history]

    def get_ensemble_history(self) -> List[Dict[str, Any]]:
        return list(self._ensemble_history)
