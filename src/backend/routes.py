...
def extract_image_from_request(img_key: str) -> Union[str, np.ndarray]:
    ...
    # Check if the request is coming as base64, file path or url from json or form data
        ...
        if not isinstance(img, str):
            raise ValueError(
                f"'{img_key}' must be a string (base64 data-URI), got {type(img).__name__}"
            )

        if not img.startswith("data:image/"):
            raise ValueError("Only base64 data-URI images are accepted. File paths and URLs are not allowed.")

        # Validate the base64 payload actually decodes to something image-sized.
        # This catches truncated / garbage data-URIs early (400) instead of letting
        # them blow up inside DeepFace (which would surface as a confusing 500).
        try:
            ...
            if len(decoded) < 64:
                raise ValueError(
                    f"image payload too small to be valid ({len(decoded)} bytes)"
                )
        except ValueError:
            raise
        except Exception as e:
            raise ValueError(f"image payload is not valid base64: {e}")
...
def analyze():
    ...
    try:
        validated = AnalyzeRequest(
            ...
            client_capture_ts=input_args.get("client_capture_ts"),
            client_frame_id=input_args.get("client_frame_id"),
        )
    ...
    try:
        ...
        # malformed base64 / unsupported image format / bad params → user error
        return make_error_response(f"Invalid image input: {input_err}", 400)

    # service.analyze may return a (body, status) tuple for handled errors.
    # Map any 5xx that originates from a malformed-image input down to 400 so
    # callers get an actionable response instead of "server crashed".
    if isinstance(demographies, tuple) and len(demographies) == 2:
        body, status = demographies
        if status >= 500 and isinstance(body, dict):
            err_text = (body.get("error") or "").lower()
            # Heuristics: anything that smells like a decode / shape failure is user-input fault.
            user_fault_markers = (
                "decode", "shape", "dimens", "channel",
                "ndim", "binascii", "could not", "unable to read",
                "invalid image", "load image",
            )
            if any(m in err_text for m in user_fault_markers):
                body = dict(body)
                body["error"] = "Could not decode the supplied image. Provide a valid base64 data-URI."
                return body, 400
        return body, status
    ...
    if session_id:
        try:
            log_metadata = {
                "client_capture_ts": validated.client_capture_ts,
                "client_frame_id": validated.client_frame_id,
                "server_received_ts": server_received_ts,
                "server_completed_ts": datetime.now(timezone.utc).isoformat(),
            ...
            log_result = session_manager.log_data(
                session_id,
                demographies,
            ...
            elif isinstance(log_result, dict) and isinstance(demographies, dict):
                if log_result.get("smoothed_emotions"):
                    demographies["smoothed_emotions"] = log_result["smoothed_emotions"]
                if log_result.get("frame_ledger"):
                    demographies["frame_ledger"] = log_result["frame_ledger"]
                if log_result.get("data_quality"):
                    demographies["data_quality"] = log_result["data_quality"]
        except Exception as e:
...
@blueprint.route("/session/stop_async", methods=["POST"])
def stop_session_async():
    """
    Asynchronously stop a session. Returns immediately with a job_id; the
    actual ``stop_session`` work runs on the background thread pool.
    Body: { "session_id": "..." }
    """
    input_args = request.get_json() or {}
    try:
    ...
    from src.backend.job_queue import submit_job
    job_id = submit_job(session_manager.stop_session, validated.session_id)
    return {"job_id": job_id, "status": "pending"}, 202


@blueprint.route("/session/pause_async", methods=["POST"])
def pause_session_async():
    """
    Asynchronously pause a session. Returns immediately with a job_id.
    Body: { "session_id": "...", "mode": "fast" | "full" }  (mode is optional)
    """
    input_args = request.get_json() or {}
    try:
        validated = SessionStopRequest(**input_args)
    ...
    # ``mode`` is accepted for forward compatibility but pause_session
    # currently reads cfg.report_mode internally; we don't override here
    # to avoid touching response shape of existing endpoints.
    from src.backend.job_queue import submit_job
    job_id = submit_job(session_manager.pause_session, validated.session_id)
    return {"job_id": job_id, "status": "pending"}, 202
...
def get_job_status(job_id: str):
    ...
    from src.backend.job_queue import get_job
    job = get_job(job_id)
    if job is None:
        return {"error": "Unknown job_id", "job_id": job_id}, 404
    return job
...
def get_session(session_id: str):
    ...
    if "error" in result:
        ...
        status = {"bad_id": 400, "not_found": 404, "db_unavailable": 503, "db_error": 500}.get(code, 500)
        return result, status
    return result


# ---------------------------------------------------------------------------
# Axis 1A — event-level endpoints
# ---------------------------------------------------------------------------

@blueprint.route("/session/<session_id>/events", methods=["GET"])
def get_session_events(session_id: str):
    """
    Re-run the event detector on the session's smoothed history with optional
    overrides. The frontend sensitivity panel uses this endpoint to explore
    parameters without restarting the session.

    Query params (all optional):
      method: "sliding" | "cusum" | "pelt" | "ensemble"
      z_threshold: float
      min_magnitude: float
      consensus_min_methods: int
    """
    method = request.args.get("method", type=str)
    z_threshold = request.args.get("z_threshold", type=float)
    min_magnitude = request.args.get("min_magnitude", type=float)
    consensus_min = request.args.get("consensus_min_methods", type=int)

    result = session_manager.get_session_events(
        session_id,
        method=method,
        z_threshold=z_threshold,
        min_magnitude=min_magnitude,
        consensus_min_methods=consensus_min,
    )
...
def get_session_clinical_metrics(session_id: str):
    ...
    suppression, incongruence) for a session. Optional payload:

        {
          "triggers": [
              {"word": "father", "frame_idx": 240, "t_sec": 14.4},
              ...
          ],
          "asr_segments": [
              {"t_start_sec": 14.0, "t_end_sec": 16.0, "valence": -0.7},
              ...
          ]
        }
    """
    ...
    if triggers is not None and not isinstance(triggers, list):
        return {"error": "triggers must be a list"}, 422
    if asr_segments is not None and not isinstance(asr_segments, list):
        return {"error": "asr_segments must be a list"}, 422

    result = session_manager.get_clinical_metrics(
        session_id, triggers=triggers, asr_segments=asr_segments
    )
    if isinstance(result, dict) and result.get("error"):
        code = result.get("code")
        status = {"bad_id": 400, "not_found": 404, "empty_session": 400, "db_unavailable": 503}.get(code, 400)
        return result, status
    return result


@blueprint.route("/session/<session_id>/clinical_report", methods=["GET"])
def get_clinical_report(session_id: str):
    """
    Generate a clinical-style report for a session. Format selected via
    ``?format=md|pdf`` (default ``md``).
    """
    fmt = (request.args.get("format") or "md").lower()
    if fmt not in {"md", "pdf"}:
        return {"error": "format must be 'md' or 'pdf'"}, 422

    from src.backend.clinical_report import build_clinical_report
    try:
    ...
    if fmt == "md":
        if request.args.get("download") in {"1", "true", "yes"}:
            ...
            filename = f"syncve_clinical_report_{session_id[:8]}.md"
            return Response(
    ...
    filename = f"syncve_clinical_report_{session_id[:8]}.pdf"
    return Response(
        pdf_bytes,
