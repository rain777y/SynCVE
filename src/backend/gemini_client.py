...
def _ensure_initialized():
    ...
    2. Vertex AI Express with API key (GEMINI_API_KEY starting with "AQ.") —
       the new Vertex-issued API keys (released 2025) route via
       ``aiplatform.googleapis.com``, so we pass ``vertexai=True`` to keep
       the SDK off the AI-Studio endpoint that those keys are blocked from.
    3. AI-Studio API key (legacy ``AIza...`` keys) — generativelanguage.

    The mode can be forced via the env var ``GENAI_MODE`` ∈
    {``vertex_express``, ``vertex_sa``, ``ai_studio``} when auto-detection
    is wrong for a given key.
    """
    ...
    try:
        ...
        forced_mode = (os.getenv("GENAI_MODE") or "").strip().lower()
        ...
        # Vertex-issued API keys begin with "AQ." (Vertex AI Express).
        # Legacy AI-Studio keys begin with "AIza".
        is_vertex_key = bool(api_key) and api_key.startswith("AQ.")
        is_ai_studio_key = bool(api_key) and api_key.startswith("AIza")

        if forced_mode == "vertex_express":
            is_vertex_key = bool(api_key)
            is_ai_studio_key = False
        elif forced_mode == "ai_studio":
            is_vertex_key = False
            is_ai_studio_key = bool(api_key)

        if forced_mode == "vertex_sa" or (forced_mode == "" and sa_path and Path(sa_path).exists()):
            if sa_path and Path(sa_path).exists():
                os.environ.setdefault("GOOGLE_APPLICATION_CREDENTIALS", sa_path)
            project = cfg.gcp_project or os.getenv("GCP_PROJECT", "sightline-hackathon")
            location = cfg.gcp_location or os.getenv("GCP_LOCATION", "us-central1")
            _genai_client = genai.Client(
                vertexai=True,
                project=project,
                location=location,
            )
            logger.info(f"GenAI client initialized via Vertex AI service account (project={project}).")
        elif is_vertex_key:
            # Vertex AI Express: API key + vertexai=True. The SDK rejects an
            # explicit location/project alongside the API key; the key
            # itself encodes routing.
            _genai_client = genai.Client(vertexai=True, api_key=api_key)
            logger.info("GenAI client initialized via Vertex AI Express (API key).")
        elif is_ai_studio_key:
            _genai_client = genai.Client(api_key=api_key)
            logger.info("GenAI client initialized via AI Studio API key.")
        elif api_key:
            # Unknown prefix — try Vertex Express first, fall back silently
            try:
                _genai_client = genai.Client(vertexai=True, api_key=api_key)
                logger.info("GenAI client initialized via Vertex Express fallback.")
            except Exception:
                _genai_client = genai.Client(api_key=api_key)
                logger.info("GenAI client initialized via AI Studio fallback.")
        else:
